...
/**
 * Crea una query de iniciativas ordenadas por sector implementando la interface IniciativaQueryBuilder.
 *
...
class Application_Model_IniciativaFinder_SectorQBuilder implements IniciativaQueryBuilder {
    ...
    function initQuery() {
        ...
        $this->_query = $dbIniciativa->select()->setIntegrityCheck(false)
                        ...
                        ->join(array('is' => 'iniciativa_sector'), 'i.id = is.id_iniciativa', array())
                        ->join(array('s' => 'sector'), 's.id = is.id_sector', array("sector" => "s.nombre"))
                        ->order('s.nombre')->order('i.nombre');
    }
    ...
    function applyFilter($filter_type, $value) {
        ...
        switch ($filter_type) {
            ...
            case self::FILTER_SECTOR:
                ...
                $this->_query->where('is.id_sector IN ( ? )', $value);
                break;
