...
/**
 * Crea una query de iniciativas ordenadas por público implementando la interface IniciativaQueryBuilder
 *
...
class Application_Model_IniciativaFinder_PublicoQBuilder implements IniciativaQueryBuilder {
    ...
    function initQuery() {
        ...
        $this->_query = $dbIniciativa->select()->setIntegrityCheck(false)
                        ...
                        ->join(array('ip' => 'iniciativa_publico'), 'i.id = ip.id_iniciativa', array())
                        ->join(array('pub' => 'publico'), 'pub.id = ip.id_publico', array("publico" => "pub.nombre"))
                        ->order('pub.nombre')->order('i.nombre');
    ...
    function applyFilter($filter_type, $value) {
        ...
        switch ($filter_type) {
            ...
            case self::FILTER_PUBLICO:
                ...
                $this->_query->where('ip.id_publico IN ( ? )', $value);
